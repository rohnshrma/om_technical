...
export default function Header() {
  ...
  return (
    <header>
      {/* Top utility bar — Call + Free Guidance, scrolls away with the page */}
      <div className="hidden bg-navy-900 sm:block">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-2 text-xs sm:px-6 lg:px-8">
          <span className="whitespace-nowrap text-navy-300">
            Serving students since {BUSINESS.foundedYear} &middot; Sector 14, Gurugram
          </span>
          <div className="flex items-center gap-4">
            <a
              href={`tel:+91${BUSINESS.phone}`}
              className="whitespace-nowrap font-semibold text-navy-100 hover:text-gold-400"
            >
              Call {BUSINESS.phone}
            </a>
            <Link
              href="/contact"
              className="whitespace-nowrap rounded-md bg-gold-500 px-3 py-1 text-xs font-semibold text-navy-900 hover:bg-gold-400"
            >
              Free Guidance
            </Link>
          </div>
      ...
      {/* Main nav row — sticky */}
      <div className="sticky top-0 z-50 border-b border-navy-100 bg-white/95 shadow-sm backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          <Link href="/" className="flex min-w-0 items-center gap-2.5">
            ...
            <span className="min-w-0 leading-tight">
              ...
              <span className="block text-xs font-medium text-gold-600 sm:hidden">
          ...
          <button
            ...
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-navy-200 text-navy-800 lg:hidden"
          >
            {menuOpen ? (
